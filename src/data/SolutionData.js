...
(function () {
    ...
    // 3. 전역 노출 설정 (기존 코드와의 호환성)
    window.solutionsData = rawData.reduce((acc, solution) => {
    ...
    window.findSolutionById = function (id) {
        if (!id) return null;
        return rawData.find(solution => solution.id === parseInt(id)) || null;
    };

    window.getAllSolutionIds = function () {
        return rawData.map(solution => solution.id);
    };

    window.solutionExists = function (id) {
        return rawData.some(solution => solution.id === parseInt(id));
    };

    window.getSolutionCount = function () {
